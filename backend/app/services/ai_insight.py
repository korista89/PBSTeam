...
def _call_gemini(system_prompt: str, user_prompt: str, max_tokens: int = 800) -> str:
    """Shared Groq API call wrapper with automatic fallback (kept the name _call_gemini for compatibility)."""
    if not GROQ_API_KEY:
        return "⚠️ Groq API Key가 설정되지 않았습니다. Vercel 환경변수에 GROQ_API_KEY를 추가해주세요."
